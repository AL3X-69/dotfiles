...
# ORIGINAL SCRIPT BY HYDE PROJECT : https://github.com/HyDE-Project/HyDE/blob/master/Configs/.local/lib/hyde/batterynotify.sh
# Edited to work on standalone + minor aesthetic changes
...
fn_status() {
    ...
    case "$battery_status" in
        ...
        *)
            if
                [[ ! -f "/tmp/batterynotify-status-fallback.$battery_status-$$" ]]
            then
                echo "Status: '==>> \"$battery_status\" <<==' Script on Fallback mode,Unknown power supply status.Also run 'ls /tmp/batterynotify-status-fallback' to see the list of lock files.*"
                touch "/tmp/batterynotify-status-fallback.$battery_status-$$"
            fi
            fn_percentage
            ;;
    esac
...
main() {
    rm -fr "$XDG_RUNTIME_DIR/batterynotify.lock"*
